...
Function AeneasArrowHold(player)
	For char.fighter = Each fighter
...
Function AeneasAttackAnimation()
	For char.fighter = Each fighter
		If char\name = "Aeneas" And char\state$ = "BASIC_ATTACK" Then
			player_2_offset = 20
			If char\player=2 Then
			...
			If (char\attackAnimFrame / 6) > char\basic_attack_frames - 1 Then
				char\attackAnimFrame = 0
				char\state$ = "NONE"
			End If
			DrawImage(char\attack1,char\attack_x,char\attack_y,(char\attackAnimFrame / 6))
			
		End If
	Next 
End Function 

Function AeneasWalk(player)
	For char.fighter = Each fighter
		If char\player = player Then
			player_2_offset = 1
			If char\player=2 Then
